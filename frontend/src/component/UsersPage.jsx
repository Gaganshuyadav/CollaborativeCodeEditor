                        ...
                        <div key={i} style={{ display:"flex", alignItems:"start", justifyContent:"start", flexDirection:"row", width:"100%", }}>
                            <div style={{ display:"flex",alignItems:"center", justifyContent:"center", color:"white", margin:"10px 4px"}}> 
                                <div style={{ width:"52px", height:"52px", borderRadius:"28%", backgroundColor: randomColor(), display:"flex", alignItems:'center', justifyContent:"center", marginLeft:"10px"}}>
                                    <Typography sx={{fontSize:"25px", textShadow:"0px 0px 10px black"}}>{listUser.name.split(" ").length<2 ? listUser.name.split(" ")[0].slice(0,1).toUpperCase() : `${listUser.name.split(" ")[0].slice(0,1).toUpperCase() + listUser.name.split(" ")[1].slice(0,1).toUpperCase()}` }</Typography>
                                </div>
                                <Typography sx={{marginLeft:"10px",letterSpacing:"0.5px"}}>{user.username===listUser.name ? `${listUser.name}(You)` : `${listUser.name}`}</Typography>
                            </div>
                        </div>
                    )
                })  
